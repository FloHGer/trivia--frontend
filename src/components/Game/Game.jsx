        ...
        if (currentUser && counter === 6) {
            ...
            (async () => {
                const res = await axios.post(
                    `${process.env.REACT_APP_BACKEND}/user/${currentUser}/games`,
                    {
                        score,
                        categories: results,
                    }
    ...
    useEffect(() => {
        ...
        allAnswers.forEach((category, i) => {
            category.forEach((question, j) => {
                ...
                if (question && j === 4) {
                    if (
                        bonus.filter((value) => value > 0).length === 5 &&
                        bonus[i] === 0
                    )
                        bonus[i] =
                            11 +
                            (1 + bonus.filter((value) => value > 0).length) *
                                100;
                    setBonus(bonus);
                    if (
                        bonus.filter((value) => value > 0).length < 5 &&
        ...
        (async () => {
            for (let i = 0; i < selectedCategories.length; i++) {
                ...
                cats[i].forEach((decoding) => {
                    ...
                    decoding.correct_answer = he.decode(
                        decoding.correct_answer
                    );
                    decoding.incorrect_answers = decoding.incorrect_answers.map(
                        (answer) => (answer = he.decode(answer))
                    );
                });
            }
            setQuestions(cats);
                        ...
                        questions.map((category, i) => {
                            return (
                                <div key={i} className={classes.game__category}>
                                    <div
                                        className={
                                            classes.game__category__title
                                        }
                                    >
                                        {/* shorten long category titles */}
                                        <div className={classes.titleHeading}>
                                            <h2>
                                                {category[0].category.startsWith(
                                                    "Entertainment: Japanese"
                                                )
                                                    ? category[0].category.slice(
                                                          24
                                                      )
                                                    : category[0].category.startsWith(
                                                          "Entertainment:"
                                    ...
                                    <ul
                                        className={
                                            classes.game__category__questions
                                        }
                                    >
                                        {/* question loop */}
                                        {questions[i].map(
